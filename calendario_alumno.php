<!DOCTYPE html>
<html lang="es">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Calendario | DOA – Gestión Educativa Inteligente</title>
    <link rel="stylesheet" href="css/estilos.css">
    <link rel="stylesheet" href="css/calendario.css">
</head>
<body class="rt-body">

    <!-- Barra superior -->
    <header class="rt-topbar">
        <a href="home_DOA.php" class="rt-topbar__logo-mobile" aria-label="DOA – Inicio">
            <img src="img/logoPrincipal_monocromatico.png" alt="DOA – Gestión Educativa Inteligente">
        </a>

        <div class="rt-topbar__spacer" aria-hidden="true"></div>

        <div class="rt-topbar__acciones">
            <!-- Muro de Anuncios -->
            <div class="ma-wrapper">
                <button id="ma-btn-notif" aria-expanded="false" aria-controls="ma-panel" class="rt-topbar__icono-btn" aria-label="Notificaciones">
                <svg viewBox="0 0 24 24" fill="none" aria-hidden="true">
                    <path d="M18 8A6 6 0 006 8c0 7-3 9-3 9h18s-3-2-3-9" stroke="currentColor" stroke-width="2"
                          stroke-linecap="round" stroke-linejoin="round"/>
                    <path d="M13.73 21a2 2 0 01-3.46 0" stroke="currentColor" stroke-width="2"
                          stroke-linecap="round" stroke-linejoin="round"/>
                </svg>
            </button>
                <span class="ma-badge" id="ma-badge" aria-label="3 notificaciones sin leer">3</span>
                <div class="ma-panel" id="ma-panel" role="dialog" aria-label="Notificaciones" aria-modal="false">
                    <div class="ma-panel__cabecera">
                        <h2 class="ma-panel__titulo">Notificaciones</h2>
                        <button class="ma-panel__cerrar" id="ma-btn-cerrar" aria-label="Cerrar notificaciones">&#x2715;</button>
                    </div>
                    <ul class="ma-lista" id="ma-lista" aria-live="polite">
                        <li>
                            <div class="ma-notif ma-notif--no-leida">
                                <span class="ma-notif__punto ma-notif__punto--tarea"></span>
                                <div class="ma-notif__cuerpo">
                                    <h3 class="ma-notif__titulo">Nueva Tarea</h3>
                                    <p class="ma-notif__desc">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Aenean euismod bibendum</p>
                                </div>
                            </div>
                        </li>
                        <li>
                            <div class="ma-notif ma-notif--no-leida">
                                <span class="ma-notif__punto ma-notif__punto--examen"></span>
                                <div class="ma-notif__cuerpo">
                                    <h3 class="ma-notif__titulo">Examen</h3>
                                    <p class="ma-notif__desc">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Aenean euismod bibendum laoreet. Proin gravida dolor sit amet lacus</p>
                                </div>
                            </div>
                        </li>
                        <li>
                            <div class="ma-notif ma-notif--no-leida">
                                <span class="ma-notif__punto ma-notif__punto--mensaje"></span>
                                <div class="ma-notif__cuerpo">
                                    <h3 class="ma-notif__titulo">Comunicado del Profesor</h3>
                                    <p class="ma-notif__desc">Lorem ipsum dolor sit amet, consectetur adipiscing elit. Aenean euismod bibendum laoreet. Proin gravida dolor sit amet lacus accumsan</p>
                                </div>
                            </div>
                        </li>
                    </ul>
                    <div class="ma-panel__pie">
                        <button class="ma-panel__marcar-todo" id="ma-marcar-todo">Marcar todo como leído</button>
                    </div>
                </div>
            </div>
            <button class="rt-topbar__icono-btn" aria-label="Ayuda">
                <svg viewBox="0 0 24 24" fill="none" aria-hidden="true">
                    <circle cx="12" cy="12" r="10" stroke="currentColor" stroke-width="2"/>
                    <path d="M9.09 9a3 3 0 015.83 1c0 2-3 3-3 3" stroke="currentColor" stroke-width="2"
                          stroke-linecap="round" stroke-linejoin="round"/>
                    <line x1="12" y1="17" x2="12.01" y2="17" stroke="currentColor" stroke-width="2"
                          stroke-linecap="round"/>
                </svg>
            </button>
            <div class="rt-perfil-wrapper">
                <button class="rt-topbar__usuario" id="rt-perfil-btn"
                        aria-expanded="false" aria-controls="rt-perfil-popup">
                    <div class="rt-topbar__usuario-info">
                        <span class="rt-topbar__usuario-nombre">Alex Rivera</span>
                        <span class="rt-topbar__usuario-rol">Estudiante</span>
                    </div>
                    <div class="rt-topbar__avatar" aria-hidden="true">
                        <svg viewBox="0 0 24 24" fill="none">
                            <circle cx="12" cy="8" r="4" stroke="currentColor" stroke-width="1.8"/>
                            <path d="M4 20c0-4 3.6-7 8-7s8 3 8 7" stroke="currentColor" stroke-width="1.8"
                                  stroke-linecap="round"/>
                        </svg>
                    </div>
                </button>
                <div class="rt-perfil-popup" id="rt-perfil-popup" hidden>
                    <div class="rt-perfil-popup__cabecera">
                        <div class="rt-perfil-popup__avatar-grande">
                            <svg viewBox="0 0 24 24" fill="none">
                                <circle cx="12" cy="8" r="4" stroke="currentColor" stroke-width="1.8"/>
                                <path d="M4 20c0-4 3.6-7 8-7s8 3 8 7" stroke="currentColor" stroke-width="1.8"
                                      stroke-linecap="round"/>
                            </svg>
                        </div>
                        <div>
                            <p class="rt-perfil-popup__nombre">Alex Rivera</p>
                            <p class="rt-perfil-popup__rol">Estudiante</p>
                        </div>
                    </div>
                    <a href="recuperar-contrasena.php" class="rt-perfil-popup__cambiar">Cambiar contraseña</a>
                    <a href="login_doa.php" class="rt-perfil-popup__logout">
                        <svg viewBox="0 0 24 24" fill="none" aria-hidden="true">
                            <path d="M9 21H5a2 2 0 01-2-2V5a2 2 0 012-2h4" stroke="currentColor" stroke-width="2"
                                  stroke-linecap="round" stroke-linejoin="round"/>
                            <polyline points="16 17 21 12 16 7" stroke="currentColor" stroke-width="2"
                                      stroke-linecap="round" stroke-linejoin="round"/>
                            <line x1="21" y1="12" x2="9" y2="12" stroke="currentColor" stroke-width="2"
                                  stroke-linecap="round"/>
                        </svg>
                        CERRAR SESIÓN
                    </a>
                </div>
            </div>
        </div>

        <button class="hamburger rt-hamburger" id="rt-hamburger" aria-label="Abrir menú"
                aria-expanded="false" aria-controls="rt-mobile-menu">
            <span class="hamburger-line"></span>
            <span class="hamburger-line"></span>
            <span class="hamburger-line"></span>
        </button>
    </header>

    <!-- MENÚ MÓVIL -->
    <div id="rt-mobile-menu" class="mobile-menu rt-mobile-menu" hidden>
        <ul class="mobile-nav-list">
            <li><a href="home_DOA.php" class="mobile-nav-link">Clases</a></li>
            <li><a href="calendario_alumno.php" class="mobile-nav-link mobile-nav-link--active" aria-current="page">Calendario</a></li>
            <li><a href="login_doa.php" class="mobile-nav-link mobile-nav-link--login">Logout</a></li>
        </ul>
    </div>

    <!-- Layout: menú lateral + contenido -->
    <div class="rt-page-wrapper">

        <!-- SIDEBAR -->
        <aside class="rt-sidebar" id="rt-sidebar" aria-label="Menú lateral">
            <nav class="rt-sidebar__nav" aria-label="Secciones del aula">
                <ul class="rt-sidebar__menu">
                    <li>
                        <a href="home_DOA.php" class="rt-sidebar__link">
                            <svg class="rt-sidebar__icon" viewBox="0 0 24 24" fill="none" aria-hidden="true">
                                <rect x="3" y="4" width="18" height="16" rx="2" stroke="currentColor" stroke-width="2"/>
                                <path d="M7 8h10M7 12h6" stroke="currentColor" stroke-width="2" stroke-linecap="round"/>
                            </svg>
                            Clases
                        </a>
                    </li>
                    <li>
                        <a href="calendario_alumno.php" class="rt-sidebar__link rt-sidebar__link--active" aria-current="page">
                            <svg class="rt-sidebar__icon" viewBox="0 0 24 24" fill="none" aria-hidden="true">
                                <rect x="3" y="5" width="18" height="16" rx="2" stroke="currentColor" stroke-width="2"/>
                                <path d="M3 10h18" stroke="currentColor" stroke-width="2"/>
                                <path d="M8 3v4M16 3v4" stroke="currentColor" stroke-width="2" stroke-linecap="round"/>
                            </svg>
                            Calendario
                        </a>
                    </li>
                </ul>
            </nav>

            <div class="rt-sidebar__footer">
                <a href="login_doa.php" class="rt-sidebar__logout">
                    <svg class="rt-sidebar__icon" viewBox="0 0 24 24" fill="none" aria-hidden="true">
                        <path d="M9 21H5a2 2 0 01-2-2V5a2 2 0 012-2h4" stroke="currentColor" stroke-width="2"
                              stroke-linecap="round" stroke-linejoin="round"/>
                        <polyline points="16 17 21 12 16 7" stroke="currentColor" stroke-width="2"
                                  stroke-linecap="round" stroke-linejoin="round"/>
                        <line x1="21" y1="12" x2="9" y2="12" stroke="currentColor" stroke-width="2"
                              stroke-linecap="round"/>
                    </svg>
                    Logout
                </a>
            </div>
        </aside>

        <!-- ÁREA PRINCIPAL -->
        <div class="rt-main-wrapper">

            <main class="cal-contenido">

                <div class="cal-card">

                    <!-- Cabecera del calendario -->
                    <div class="cal-header">
                        <div class="cal-header__izq">
                            <button class="cal-nav-btn" id="cal-prev" aria-label="Anterior">
                                <svg viewBox="0 0 24 24" fill="none" aria-hidden="true">
                                    <polyline points="15 18 9 12 15 6" stroke="currentColor" stroke-width="2"
                                              stroke-linecap="round" stroke-linejoin="round"/>
                                </svg>
                            </button>
                            <span class="cal-titulo" id="cal-titulo">Mayo 2026</span>
                            <button class="cal-nav-btn" id="cal-next" aria-label="Siguiente">
                                <svg viewBox="0 0 24 24" fill="none" aria-hidden="true">
                                    <polyline points="9 18 15 12 9 6" stroke="currentColor" stroke-width="2"
                                              stroke-linecap="round" stroke-linejoin="round"/>
                                </svg>
                            </button>
                        </div>

                        <!-- Leyenda tipos de evento -->
                        <div class="cal-header__der">
                            <div class="cal-leyenda" aria-label="Leyenda">
                                <span class="cal-leyenda__item">
                                    <span class="cal-leyenda__punto cal-leyenda__punto--tarea"></span>
                                    Tareas
                                </span>
                                <span class="cal-leyenda__item">
                                    <span class="cal-leyenda__punto cal-leyenda__punto--examen"></span>
                                    Exámenes
                                </span>
                                <span class="cal-leyenda__item">
                                    <span class="cal-leyenda__punto cal-leyenda__punto--evento"></span>
                                    Eventos
                                </span>
                            </div>
                        </div>
                    </div>

                    <!-- Mes (pantallas anchas) -->
                    <div class="cal-mes" id="cal-mes">
                        <table class="cal-grid" role="grid" aria-label="Calendario mensual">
                            <thead>
                                <tr>
                                    <th scope="col">Lun</th>
                                    <th scope="col">Mar</th>
                                    <th scope="col">Mié</th>
                                    <th scope="col">Jue</th>
                                    <th scope="col">Vie</th>
                                    <th scope="col" class="cal-weekend">Sáb</th>
                                    <th scope="col" class="cal-weekend">Dom</th>
                                </tr>
                            </thead>
                            <tbody id="cal-cuerpo">
                                <!-- Generado por JS -->
                            </tbody>
                        </table>
                    </div>

                    <!-- Vista SEMANA VERTICAL (móvil, generada por JS) -->
                    <div class="cal-semana-vertical" id="cal-semana-vertical">
                        <!-- Generado por JS -->
                    </div>

                </div><!-- /cal-card -->

            </main>

            <footer class="pie-pagina rt-footer">
                <div class="contenedor pie-pagina__interior">
                    <a href="index.php" class="pie-pagina__logo">
                        <img src="img/logo-gti-removebg-preview.png" alt="GTI – Grado en Tecnologías Interactivas">
                    </a>
                    <p class="pie-pagina__derechos">Todos los derechos reservados 2026&copy;</p>
                </div>
            </footer>

        </div><!-- /rt-main-wrapper -->

    </div><!-- /rt-page-wrapper -->

    <!-- Barra inferior móvil -->
    <nav class="rt-bottom-tabs" aria-label="Navegación inferior">
        <a href="home_DOA.php" class="rt-bottom-tab">Clases</a>
        <a href="calendario_alumno.php" class="rt-bottom-tab rt-bottom-tab--activo" aria-current="page">Calendario</a>
    </nav>

    <script>
        // Todo en una función para no llenar el scope global.
        (function () {
            // Ejemplo mayo 2026; clave = fecha en formato año-mes-día.
            var EVENTS = {
                '2026-05-04': [{ texto: 'Entrega Prov.', tipo: 'tarea' }],
                '2026-05-07': [{ texto: 'Reunión equipo', tipo: 'evento' }],
                '2026-05-08': [{ texto: 'Examen Mat.', tipo: 'examen' }],
                '2026-05-12': [
                    { texto: 'Leer Cap 4 Lit.', tipo: 'tarea' },
                    { texto: 'Ejercicios Fís.', tipo: 'tarea' }
                ],
                '2026-05-14': [{ texto: 'Charla sobre IA', tipo: 'evento' }],
                '2026-05-15': [{ texto: 'Control Lec.', tipo: 'examen' }],
                '2026-05-19': [{ texto: 'Presentación', tipo: 'tarea' }],
                '2026-05-20': [{ texto: 'Jornada puertas abiertas', tipo: 'evento' }],
                '2026-05-28': [{ texto: 'Examen Final', tipo: 'examen' }]
            };

            var NOMBRES_MESES = [
                'Enero','Febrero','Marzo','Abril','Mayo','Junio',
                'Julio','Agosto','Septiembre','Octubre','Noviembre','Diciembre'
            ];
            var DIAS_SEMANA_CORTO = ['Lun','Mar','Mié','Jue','Vie','Sáb','Dom'];

            var hoy         = new Date();
            var mesActual   = new Date(hoy.getFullYear(), hoy.getMonth(), 1);
            var semanaActual = obtenerLunesDeSemana(hoy);

            function pad(n) { return n < 10 ? '0' + n : '' + n; }

            function claveEvento(y, m, d) {
                return y + '-' + pad(m + 1) + '-' + pad(d);
            }

            // Lunes de la semana de una fecha (calendario tipo ISO).
            function obtenerLunesDeSemana(fecha) {
                var d   = new Date(fecha);
                var dow = d.getDay();
                d.setDate(d.getDate() + ((dow === 0) ? -6 : 1 - dow));
                return d;
            }

            function esHoy(fecha) {
                return fecha.getFullYear() === hoy.getFullYear() &&
                       fecha.getMonth()    === hoy.getMonth()    &&
                       fecha.getDate()     === hoy.getDate();
            }

            function crearPastillaEvento(ev) {
                var span = document.createElement('span');
                span.className = 'cal-evento cal-evento--' + ev.tipo;
                span.textContent = ev.texto;
                span.title = ev.texto;
                return span;
            }

            function renderizarMes() {
                var anio = mesActual.getFullYear();
                var mes  = mesActual.getMonth();

                document.getElementById('cal-titulo').textContent =
                    NOMBRES_MESES[mes] + ' ' + anio;

                var primerDia = new Date(anio, mes, 1).getDay();
                primerDia = (primerDia === 0) ? 6 : primerDia - 1;

                var diasEnMes      = new Date(anio, mes + 1, 0).getDate();
                var diasMesAnterior = new Date(anio, mes, 0).getDate();

                var cuerpo = document.getElementById('cal-cuerpo');
                cuerpo.innerHTML = '';

                var dia = 1, diaSig = 1;
                var totalCeldas = Math.ceil((primerDia + diasEnMes) / 7) * 7;

                for (var i = 0; i < totalCeldas; i++) {
                    if (i % 7 === 0) cuerpo.appendChild(document.createElement('tr'));

                    var fila = cuerpo.lastElementChild;
                    var td   = document.createElement('td');
                    var col  = i % 7;
                    var finde = col >= 5;

                    if (i < primerDia) {
                        var numPrev = diasMesAnterior - primerDia + i + 1;
                        td.classList.add('cal-otro-mes');
                        if (finde) td.classList.add('cal-weekend-cell');
                        td.innerHTML = '<span class="cal-dia-num">' + numPrev + '</span>';

                    } else if (dia <= diasEnMes) {
                        if (finde) td.classList.add('cal-weekend-cell');
                        var hoyAhora = (anio === hoy.getFullYear() && mes === hoy.getMonth() && dia === hoy.getDate());
                        td.innerHTML = '<span class="cal-dia-num' + (hoyAhora ? ' cal-dia-num--hoy' : '') + '">' + dia + '</span>';

                        var clave = claveEvento(anio, mes, dia);
                        if (EVENTS[clave]) {
                            var contenedor = document.createElement('div');
                            contenedor.className = 'cal-eventos';
                            EVENTS[clave].forEach(function (ev) {
                                contenedor.appendChild(crearPastillaEvento(ev));
                            });
                            td.appendChild(contenedor);
                        }
                        dia++;

                    } else {
                        td.classList.add('cal-otro-mes');
                        if (finde) td.classList.add('cal-weekend-cell');
                        td.innerHTML = '<span class="cal-dia-num">' + diaSig + '</span>';
                        diaSig++;
                    }

                    fila.appendChild(td);
                }
            }

            function renderizarSemanaVertical() {
                var inicio = new Date(semanaActual);
                var fin    = new Date(semanaActual);
                fin.setDate(fin.getDate() + 6);

                document.getElementById('cal-titulo').textContent =
                    inicio.getDate() + ' – ' + fin.getDate() + ' ' +
                    NOMBRES_MESES[fin.getMonth()] + ' ' + fin.getFullYear();

                var contenedor = document.getElementById('cal-semana-vertical');
                contenedor.innerHTML = '';

                for (var i = 0; i < 7; i++) {
                    var d     = new Date(semanaActual);
                    d.setDate(d.getDate() + i);
                    var finde = i >= 5;

                    var fila = document.createElement('div');
                    fila.className = 'cal-dia-fila' + (finde ? ' cal-dia-fila--finde' : '');

                    var cab = document.createElement('div');
                    cab.className = 'cal-dia-fila__cabecera';

                    var nombre = document.createElement('span');
                    nombre.className = 'cal-dia-fila__nombre' + (finde ? ' cal-dia-fila__nombre--finde' : '');
                    nombre.textContent = DIAS_SEMANA_CORTO[i];

                    var num = document.createElement('span');
                    num.className = 'cal-dia-num' + (esHoy(d) ? ' cal-dia-num--hoy' : '');
                    num.textContent = d.getDate();

                    cab.appendChild(nombre);
                    cab.appendChild(num);

                    var areaEventos = document.createElement('div');
                    areaEventos.className = 'cal-dia-fila__eventos';

                    var clave = claveEvento(d.getFullYear(), d.getMonth(), d.getDate());
                    if (EVENTS[clave]) {
                        EVENTS[clave].forEach(function (ev) {
                            areaEventos.appendChild(crearPastillaEvento(ev));
                        });
                    }

                    fila.appendChild(cab);
                    fila.appendChild(areaEventos);
                    contenedor.appendChild(fila);
                }
            }

            var mq = window.matchMedia('(max-width: 720px)');

            function actualizarVista(esMobil) {
                var elMes     = document.getElementById('cal-mes');
                var elSemanaV = document.getElementById('cal-semana-vertical');

                if (esMobil) {
                    elMes.classList.add('oculto-movil');
                    elSemanaV.classList.add('activa');
                    renderizarSemanaVertical();
                } else {
                    elMes.classList.remove('oculto-movil');
                    elSemanaV.classList.remove('activa');
                    renderizarMes();
                }
            }

            if (mq.addEventListener) {
                mq.addEventListener('change', function (e) { actualizarVista(e.matches); });
            } else {
                mq.addListener(function (e) { actualizarVista(e.matches); });
            }

            document.getElementById('cal-prev').addEventListener('click', function () {
                if (mq.matches) {
                    semanaActual.setDate(semanaActual.getDate() - 7);
                    renderizarSemanaVertical();
                } else {
                    mesActual.setMonth(mesActual.getMonth() - 1);
                    renderizarMes();
                }
            });

            document.getElementById('cal-next').addEventListener('click', function () {
                if (mq.matches) {
                    semanaActual.setDate(semanaActual.getDate() + 7);
                    renderizarSemanaVertical();
                } else {
                    mesActual.setMonth(mesActual.getMonth() + 1);
                    renderizarMes();
                }
            });

            var perfilBtn   = document.getElementById('rt-perfil-btn');
            var perfilPopup = document.getElementById('rt-perfil-popup');
            if (perfilBtn && perfilPopup) {
                perfilBtn.addEventListener('click', function (e) {
                    e.stopPropagation();
                    var abierto = !perfilPopup.hidden;
                    perfilPopup.hidden = abierto;
                    perfilBtn.setAttribute('aria-expanded', String(!abierto));
                });
                document.addEventListener('click', function () {
                    perfilPopup.hidden = true;
                    perfilBtn.setAttribute('aria-expanded', 'false');
                });
                perfilPopup.addEventListener('click', function (e) { e.stopPropagation(); });
            }

            var hamburger  = document.getElementById('rt-hamburger');
            var mobileMenu = document.getElementById('rt-mobile-menu');
            if (hamburger && mobileMenu) {
                hamburger.addEventListener('click', function () {
                    var abierto = !mobileMenu.hidden;
                    mobileMenu.hidden = abierto;
                    hamburger.setAttribute('aria-expanded', String(!abierto));
                });
                mobileMenu.querySelectorAll('a').forEach(function (a) {
                    a.addEventListener('click', function () {
                        mobileMenu.hidden = true;
                        hamburger.setAttribute('aria-expanded', 'false');
                    });
                });
            }

            actualizarVista(mq.matches);

        }());
    </script>

    <!-- auth-demo: quitar cuando haya login real -->
    <script src="js/auth-demo.js"></script>
    <script>AuthDemo.inicializar({ rol: 'alumno' });</script>

    <script src="js/script.js" defer></script>
</body>
</html>
